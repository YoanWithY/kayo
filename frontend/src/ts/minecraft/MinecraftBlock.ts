...
export class ParsedBlockModelElement {
	...
	public constructor(parse: any) {
		...
		if (parse.shade !== undefined) this.shade = parse.shade;

		for (const faceKey in parse.faces) {
			const parentFace = parse.faces[faceKey];
			const newFace: any = {};
			for (const parentFaceKey in parentFace) newFace[parentFaceKey] = parentFace[parentFaceKey];
	...
	public constructor(parsedElement: ParsedBlockModelElement, parsedBlockStateModel: ParsedBlockStateModel) {
		...
		for (const key of elementFaceKeys) {
			if (faces[key]) {
				...
				this[key] = builtFace;
				if (!builtFace.texture || !builtFace.texture.isOpaque()) this.isOpaque = false;
			} else {
				this.isOpaque = false;
			}
		}
...
export class ParsedBlockModel {
	...
	public expandRecursevly(resourcePack: ResourcePack) {
		...
		if (parentName === undefined || parentName === "") {
			if (this.parsed.ambientocclusion !== undefined) this.ambientocclusion = this.parsed.ambientocclusion;

			if (this.parsed.display !== undefined) this.display = this.parsed.display;

			if (this.parsed.elements !== undefined) {
				for (const e of this.parsed.elements) this.elements.push(new ParsedBlockModelElement(e));
			}
		...
		if (!parent.isExpand) parent.expandRecursevly(resourcePack);

		if (this.parsed.ambientocclusion !== undefined) this.ambientocclusion = this.parsed.ambientocclusion;
		else this.ambientocclusion = parent.ambientocclusion;

		if (this.parsed.display !== undefined) this.display = this.parsed.display;
		else this.display = parent.display;

		if (this.parsed.elements !== undefined) {
			for (const e of this.parsed.elements) this.elements.push(new ParsedBlockModelElement(e));
		} else {
			for (const e of parent.elements) this.elements.push(new ParsedBlockModelElement(e));
		}
	...
	private resolveTextureVariable(res: ResourcePack, key: string): MinecraftTexture | undefined {
		...
		if (value[0] === "#" && value.length > 1) {
			...
			if (key == variableName) return undefined;
	...
	public resolveTextures(res: ResourcePack) {
		for (const key in this.textureStrings) this.resolveTextureVariable(res, key);
		...
		for (const e of this.elements) {
			...
			for (const faceKey in faces) {
				...
				if (faces[faceName] !== undefined) {
					...
					if (tex && !(tex instanceof MinecraftTexture)) {
						...
						if (mTex) faces[faceName].texture = mTex;
					}
				}
			}
			...
			const from = e.from;
			const to = e.to;
			if (faces.down && !faces.down.uv) faces.down.uv = [from[0], 16 - to[2], to[0], 16 - from[2]];
			if (faces.up && !faces.up.uv) faces.up.uv = [from[0], from[2], to[0], to[2]];
			if (faces.north && !faces.north.uv) faces.north.uv = [16 - to[0], 16 - to[1], 16 - from[0], 16 - from[1]];
			if (faces.east && !faces.east.uv) faces.east.uv = [16 - to[2], 16 - to[1], 16 - from[2], 16 - from[1]];
			if (faces.south && !faces.south.uv) faces.south.uv = [from[0], 16 - to[1], to[0], 16 - from[1]];
			if (faces.west && !faces.west.uv) faces.west.uv = [from[2], 16 - to[1], to[2], 16 - from[1]];
		}
	}
	...
	public build(
		...
		for (const element of this.elements) {
			for (const key of elementFaceKeys) {
				...
				if (useCulling && neighbor !== undefined && neighbor.isFullOpaque) continue;
...
export class ParsedBlockStateModel {
	...
	public constructor(res: ResourcePack, parsed: any) {
		...
		this.model = m;
		if (parsed.x !== undefined) this.x = parsed.x;
		if (parsed.y !== undefined) this.y = parsed.y;
		if (parsed.weight !== undefined) this.weight = parsed.weight;
		if (parsed.uvlock !== undefined) this.uvlock = parsed.uvlock;
...
class OrMatcher extends Matcher {
	...
	public constructor(parsed: any) {
		...
		for (const obj of parsed) {
			...
			for (const key in obj) smArray.push(new StateMatcher(key, obj[key]));
	...
	public matches(properties: any): boolean {
		for (const smArr of this.conditions) {
			...
			for (const m of smArr) {
				if (!m.matches(properties)) matches = false;
			}
			if (!matches) continue;
			return true;
		}
...
export class BlockState {
	...
	public getBlockStateModelByProperties(properties: any): ParsedBlockStateModel[] | undefined {
		if (this.variants) {
			...
			if (Array.isArray(models)) return [models[Math.floor(Math.random() * models.length)]];
			else if (models !== undefined) return [models];
		}
		...
		if (this.multipart) {
			...
			for (const mpCase of this.multipart) {
				if (mpCase.when.matches(properties)) {
					...
					if (Array.isArray(apply)) tempArray.push(apply[Math.floor(Math.random() * apply.length)]);
					else tempArray.push(apply);
				}
			}
			return tempArray;
		}
