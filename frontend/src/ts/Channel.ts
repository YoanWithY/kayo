...
export class ChannelToMain {
	...
	postUIToggleBoolean(booleanStateVaribaleGUID: GUID) {
		const msg: UIToggleBooleanCM = {
			type: "ui",
			operation: "toggle boolean",
			targetVariable: booleanStateVaribaleGUID,
		};
		this.channel.postMessage(msg);
	}

	postUISetSelect(selectStateVaribaleGUID: GUID, value: string | number) {
		const msg: UISetSelectCM = {
			type: "ui",
			operation: "set select",
			targetVariable: selectStateVaribaleGUID,
			value: value,
		};
		this.channel.postMessage(msg);
	}
