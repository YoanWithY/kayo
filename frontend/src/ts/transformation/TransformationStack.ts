...
export default class TransformationStack extends Array implements Transformation {
    ...
    getTransformationMatrix(): mat4 {
        ...
        for (let i = this.length - 1; i >= 0; i--)
            ret = ret.mult(this[i].getTransformationMatrix());

        ret = ret.mult(this.translate.getTransformationMatrix()).mult(this.rotate.getTransformationMatrix()).mult(this.scale.getTransformationMatrix());
        return ret;
    }
    ...
    getInverseTransformationMatrix(): mat4 {
        ...
        for (let i = 0; i < this.length; i++)
            ret = ret.mult(this[i].getInverseTransformationMatrix());
        return ret;
    }
