...
export default class RessourcePane extends BasicPane {
	...
	private buildCallback = () => {
		if (this.tree) this.removeChild(this.tree);
		const gpux = this.kayo.project.gpux;
		const gpu = gpux.gpu;
	...
	public static createUIElement(win: Window, kayo: Kayo): RessourcePane {
		const p = super.createUIElement(win, kayo, ressourcePaneTemplate) as RessourcePane;
		p._win = win;
		return p;
	}
