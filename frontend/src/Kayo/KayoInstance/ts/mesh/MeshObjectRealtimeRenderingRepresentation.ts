...
export class MeshObjectRealtimeRenderingRepresentation
	...
	public constructor(kayo: KayoInstance, representationConcept: RealtimeRenderer, representationSubject: MeshObject) {
	...
	private _setup() {
		...
		for (let i = 0; i < this._representationSubject.mesh.materials.size(); i++) {
			...
			let material = this._kayo.project.materials.find((m) => m.name == matName);
			if (!material) {
				console.warn(`No material named "${matName}"!`);
				matName = "default";
			}
			// eslint-disable-next-line local/no-anonymous-arrow-function
			material = this._kayo.project.materials.find((m) => m.name == matName);
			if (!material) {
