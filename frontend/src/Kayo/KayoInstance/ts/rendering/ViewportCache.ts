...
export class RealtimeViewportCache {
	...
	public constructor(kayo: KayoInstance, viewport: UI3DViewport, config: RealtimeConfigObject) {
		this._kayo = kayo;
		this.viewport = viewport;
		this.gpuDevice = this._kayo.gpux.gpuDevice;
		this.querySet = this.gpuDevice.createQuerySet({
			label: `time stamp query set for viewport`,
			type: "timestamp",
			count: 10,
		});
		this.timeStempBufferResolve = this.gpuDevice.createBuffer({
			label: `time stemp querey resolve buffer for viewport`,
		...
		this.timeStempMapBuffer = this.gpuDevice.createBuffer({
			label: `time stemp map buffer for viewport`,
			size: this.timeStempBufferResolve.size,
			usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
		});

		this.reconfigureContext(config);
	}

	public conditionalFrambebufferUpdate(config: RealtimeConfigObject) {
	...
	private conditionalColorAttachmentUpdate(w: number, h: number, config: RealtimeConfigObject) {
		const currentFormat = this._kayo.gpux.getSwapChainFormat(config.bitDepth);
		const msaa = config.msaa;
		if (
			w === this.prevWidth &&
			h === this.prevHeight &&
			msaa === this.prevMSAA &&
	...
	public reconfigureContext(config: RealtimeConfigObject) {
		...
		context.configure({
			...
			format: this._kayo.gpux.getSwapChainFormat(config.bitDepth),
			colorSpace: config.colorSpace as PredefinedColorSpace,
			toneMapping: { mode: config.toneMappingMode as GPUCanvasToneMappingMode },
			usage: GPUTextureUsage.RENDER_ATTACHMENT,
		});
	}
