...
export class PTPTrackLog {
    ...
    public prepConnection(peerConnection: RTCPeerConnection, identity: Identity) {
        const incommingTrackListener = (e: RTCTrackEvent) => {
            ...
            const t = peerConnection.getTransceivers().find(t => t.receiver.track == e.track);
            if (t) {
                (t.receiver as any).playoutDelayHint = 0;
            }
            console.log("incomming track", t);
        }
        ...
        const outgoindListener = (outgoingStream: OutgoingStream) => {
            ...
            p.encodings[0].maxBitrate = 100_000_000;
            p.encodings[0].maxFramerate = 30;
            p.encodings[0].scaleResolutionDownBy = 1;
            p.encodings[0].priority = "medium";
            p.encodings[0].networkPriority = "medium";
            p.degradationPreference = "maintain-framerate" as RTCDegradationPreference;
            console.log("try apply:", p);
    ...
    private _outgoingStreamReadyCallback = (stream: MediaStream) => {
        for (const track of stream.getTracks()) {
            ...
            track.applyConstraints({ width: { exact: caps.width?.max }, height: { exact: caps.height?.max }, frameRate: { exact: caps.frameRate?.max } });
