...
const minorRange = 200;
const majorRange = 500;
...
const float mediumStartP = ${(minorRange + 10).toFixed(1)}, mediumStartN = -mediumStartP + 10.0;
const float largeStartP = ${(majorRange + 100).toFixed(1)}, largeStartN = -largeStartP + 100.0;
...
void main(){
    ...
    maxFade = (isLarge(worldPos) ? 1500.0 : isMedium(worldPos) ? ${(majorRange * 1.5).toFixed(1)} :  ${(minorRange * 1.5).toFixed(1)}) * sqrt(linearStep(0.0, 16.0, abs(cameraPosition.z))); 
...
export default class Grid3D {
    ...
    static count = 0;
    static lineGenShader = new ShaderProgram(lineSegmentGeometryGenerationVertexShader, Shader3D.emptyFragmentShader, ["posOff", "fac"], ["p0OL", "p0OM", "p0OR", "p1OL", "p1OM", "p1OR"], gl.INTERLEAVED_ATTRIBS);
    static renderShader = new ShaderProgram(vertexShaderCode, fragmentShaderCode, ["pxLineWidth"]);
    ...
    static {
        ...
        for (let d = 1; d <= minorRange; d++)
            this.appendD(pos, weight, d, 1);

        for (let d = minorRange + 10; d <= majorRange; d += 10)
            this.appendD(pos, weight, d, 10);

        for (let d = majorRange + 100; d <= 1000; d += 100)
